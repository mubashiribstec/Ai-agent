...
Precedence (highest wins):
    1. Environment variables (``NEXUS_*`` and provider keys)
    2. User config at ``$NEXUS_HOME/config.yaml`` (default ``~/.nexus``)
...
def nexus_home() -> Path:
    """Return the Nexus data directory, creating it if needed."""
    home = Path(os.environ.get("NEXUS_HOME", Path.home() / ".nexus"))
...
class Config:
    ...
    home: Path = field(default_factory=nexus_home)
...
def load_config(overrides: dict[str, Any] | None = None) -> Config:
    ...
    user_cfg = nexus_home() / "config.yaml"
    data = _deep_merge(data, _load_yaml(user_cfg))

    # Environment overrides
    if model := os.environ.get("NEXUS_MODEL"):
        data["model"] = model
    if rmodel := os.environ.get("NEXUS_REFLECTION_MODEL"):
        data["reflection_model"] = rmodel
    if emodel := os.environ.get("NEXUS_EMBEDDING_MODEL"):
